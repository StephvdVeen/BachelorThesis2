...
public class Objective_check {
	...
	public Objective_check(Route initial, double[][] distances, double M, double Q) {
		...
		for (int i = 0; i < initial_cust.size() -1; i++) {
			if (y[i][0] != 0) {
				weight = weight + y[i][0];	
				System.out.println("demand" + y[i][0]);
		...
		for (int i = 1 ; i < y.length -1; i++) {
			from = initial_cust.get(i-1).getIndex() -1;
			to = initial_cust.get(i).getIndex() -1;
